...
public class StockServiceImpl implements IStockService {
  ...
  public ServerResponse getStock(int pageNum, int pageSize, String keyWords, String stockPlate, String stockType, HttpServletRequest request) {
    ...
    if (stockList.size() > 0) {
      Set<String> collect = stockList.stream().map(e -> "s_"+e.getStockGid()).collect(Collectors.toSet());
      String stockGidList = StringUtils.join(collect, ",");
      List<QqMiniData> simpleMarketList = QqStockApi.getSimpleAllMarket(stockGidList);
      for (int i=0;i<stockList.size();i++) {
        Stock stock=stockList.get(i);
        for(QqMiniData qqMiniData:simpleMarketList){
          if(stock.getStockCode().equals(qqMiniData.getCode())){
            //StockListVO stockListVO = SinaStockApi.assembleStockListVO(SinaStockApi.getSinaStock(stock.getStockGid()));
            StockListVO stockListVO=new StockListVO();
            stockListVO.setHcrate(new BigDecimal(qqMiniData.getZdfd()));
            stockListVO.setNowPrice(qqMiniData.getNowPrice());
            stockListVO.setPreclose_px(qqMiniData.getZd());
